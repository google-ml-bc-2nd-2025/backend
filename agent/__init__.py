...
def answer_with_agent(question: str, model_name=DEFAULT_MODEL) -> str:
    """
...
def streaming_agent_execution(question: str, model_name=DEFAULT_MODEL):
    """
    LangGraph 에이전트 실행 과정을 콘솔에 스트리밍 출력 (디버깅용)
    """
    print("=== 에이전트 실행 시작 ===\n")
    print(f"📝 질문: {question}\n")
    ...
    for event in agent.stream(initial_state):
        ...
        if node == "check_game_resource":
            ...
            print("\n🔍 요청 분석 결과:")
            print(f"게임 리소스 요청: {'예' if state.get('is_game_resource_request') else '아니오'}")
            if state.get("is_game_resource_request"):
